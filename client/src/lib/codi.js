export const voiceCommandsRegex = {
  ...
  readSolution: /.*(what is|read) (this|the)?( )?(solution|sequence|commands).*/,
  whatIsThis: /.*(what is|what's) (this|that).*/,
  computeSequence: /.*(compute|check|verify) (solution|sequence|commands).*/,
  recognizeBoard: /.*(recognize) (board|map).*/,
  recognizeSolution: /.*(recognize) (solution|sequence|commands).*/,
  executeSolution: /.*(go|start|execute) (solution|sequence|commands) (codi|kodi|buddy|cody|robot).*/,
  saveMapToDatabase: /.*(save|send) (map|board).*/,
  saveSolutionToDatabase: /.*(save|send) (solution|sequence|commands).*/,
